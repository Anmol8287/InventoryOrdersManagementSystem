...
def list_products(db: Session, search: str | None = None, skip: int = 0, limit: int = 100) -> list[Product]:
    query = db.query(Product).order_by(Product.created_at.desc())
    if search:
        like = f"%{search}%"
        query = query.filter((Product.product_name.ilike(like)) | (Product.sku.ilike(like)))
